...
**✅ 按属性特效已于 2026-09-06 人工 Play 验收通过**(用户:「没问题 验收通过」)。这同时证明了动态加载那条分支确实在走,不是一直吃兜底。

> **VFX 三层(飘字 / 粒子 / 震屏)至此全部关闭**,自动化 + 人工两头都过。
...
**✅ 人工 Play 验收已通过(2026-09-06)** —— 用户实测「没有太大问题」,WASD / 鼠标 / 1–5 / Tab / 右键瞄准 / 射程内无敌人不刷 `Accessed None` / 敌方 AI 移动均未见异常。**队列 #5 完整关闭。**
备份仍在:`Content/Maps/BP_Unit.uasset.bak_20260906`(清理前原件)+ 每岛一份 `.bak_<岛头名>`,稳定一段时间后可清。
