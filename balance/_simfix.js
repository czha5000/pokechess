...
const {typeMult,SKILLS,LEARN,POOL,WILD,ELITE,CH_BOSS,EVO,EEVEE_FORMS,RELICS,
  COLS,ROWS,TERRAIN,PSTART,ESLOTS,MAXLV,THRESH,STAGE_LV,EVO_BONUS,CH_SCALE}=G;

const EH=+(process.env.EH||1.8), BH=+(process.env.BH||2.4), EC=+(process.env.EC||0), DEPTOT=+(process.env.DEPTOT||5);
...
function rStat(u){for(const r of EQUIP)if(r.statMod)r.statMod(u);}
...
function baseDmg(att,def,sk){const m=typeMult(sk.type,def.type);if(m===0)return{d:0,m:0};let d=Math.max(1,Math.round(att.atk*sk.mult*m)-def.def-cover(def));d=Math.round(d*rDmg(att,sk));return{d:Math.max(1,d),m};}
...
function critRate(att,def,sk){return Math.max(0,Math.min(100,Math.round(sk.crit+att.skl-def.lck)));}
...
function strike(att,def,skKey){const s=SKILLS[skKey]||skKey;const times=(doubles(att,def)&&s.kind!=='aoe')?2:1;let dealt=0;
  for(let i=0;i<times;i++){if(def.hp<=0)break;const b=baseDmg(att,def,s);
    if(b.m===0){if(att.player)tally('immune');break;}
...
function poolEntry(p){const e={key:p.key,type:p.type,hero:!!p.hero,lv:1,exp:0,stage:0,maxhp:p.hp,atk:p.atk,def:p.def,spd:p.spd,skl:p.skl,lck:p.lck,mov:p.mov,rng:p.rng,skills:[(LEARN[p.key]||['basic'])[0]]};if(STARTLV>1)metaBump(e,STARTLV);return e;}
function mkBattleUnit(src,x,y){const u=Object.assign({},src);u.player=true;u.src=src;u.x=x;u.y=y;u.hp=u.maxhp;u.skills=src.skills.slice();u.transformed=false;rStat(u);return u;}
...
function battle(pool,deploy,nodeType,chapter){
  ...
  const sync=()=>units.filter(u=>u.player&&u.hp>0).forEach(u=>{const s=u.src;s.lv=u.lv;s.exp=u.exp;s.stage=u.stage;s.maxhp=u.maxhp;s.atk=u.atk;s.def=u.def;s.spd=u.spd;s.skl=u.skl;if(!s.hero)s.skills=u.skills.slice();});
  let turns=0;
  while(turns<40){turns++;
    const order=units.filter(u=>u.hp>0).slice().sort((a,b)=>(b.spd-a.spd)||((a.player?0:1)-(b.player?0:1)));
    for(const u of order){if(u.hp<=0)continue;u.player?actPlayer(units,u):actEnemy(units,u);if(!eAlive()||!pAlive())break;}
...
function runOnce(){let pool=POOL.map(poolEntry);run={chapter:1};
  ...
  for(let ch=1;ch<=3;ch++){rec.chapter=ch;
    for(let col=0;col<6;col++){let type=col===5?'boss':col===0?'battle':['battle','battle','elite','event','rest'][ri(5)];
      if(type==='event'||type==='rest')continue;
...
const TH=Object.values(M.hits).reduce((a,b)=>a+b,0);
console.log('===== 平衡台 v2策略 · '+N+' 轮 =====');
console.log('通关率:',pct(agg.wins,N),'| 各章:',pct(agg.clear[1],N),pct(agg.clear[2],N),pct(agg.clear[3],N));
console.log('平均阵亡/轮:',(agg.deaths/N).toFixed(2),'| 回合/场:',(agg.turns/agg.battles).toFixed(1),'| Boss回合:',avg(agg.bossT[1]).toFixed(1),avg(agg.bossT[2]).toFixed(1),avg(agg.bossT[3]).toFixed(1));
console.log('失败点:');Object.entries(agg.diedAt).sort((a,b)=>b[1]-a[1]).forEach(([k,v])=>console.log('  '+k+': '+pct(v,N)));
console.log('克制 super/neutral/resist:',pct(M.hits.super||0,TH),pct(M.hits.neutral||0,TH),pct(M.hits.resist||0,TH));
