...
function relicDmgMult(att,sk){let m=1;if(att.side==='player')for(const r of run.relics){if(r.dmgMult)m*=r.dmgMult(att,sk);}return m;}
...
function relicCritAdd(att){let a=0;if(att.side==='player')for(const r of run.relics){if(r.critAdd)a+=r.critAdd;}return a;}
