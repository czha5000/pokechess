...
function spawnEnemies(n){const out=[];const ks=Object.keys(WILD);
  ...
  return out;}
function startBattle(n){units=[];battleCaptures=[];selected=null;clearSel();window.inBossBattle=(n&&n.type==='boss');if(typeof setBattleMap==='function')setBattleMap(null,run.chapter);run._battleOver=false;run.obj=(n.type==='battle'&&n.obj)?n.obj:'clear';run.objN=n.objN||3;run.round=0;run.objX=null;run.objY=null;
...
function onBattleWin(){if(run._battleOver)return;run._battleOver=true;window.inBossBattle=false;run.gold=(run.gold||0)+12+run.chapter*3;log(`💰 获得金币 +${12+run.chapter*3}(共 ${run.gold})`,'#ffd95a');
  ...
  battleCaptures.forEach(e=>run.pool.push(poolEntryFromEnemy(e,ENEMY_LV[run.chapter]||1))); // 收服按本章基准等级入池,后期不再捡Lv1垃圾
  const n=nodeById(run.cur);if(n)n.done=true;
  log(`<b>胜利！</b>${battleCaptures.length?' 收服 '+battleCaptures.map(c=>c.name).join('、'):''}`,'#7fe0a0');
  const cont=()=>{
    if(n&&n.type==='boss'){
      if(run.chapter>=3){var _ev=(typeof STORY!=='undefined')?STORY.winVideo:null;if(_ev&&typeof showVideo==='function')showVideo(_ev,function(){runOver(true);});else runOver(true);return;}
      run.pool.forEach(m=>m.curHp=m.maxhp);log('🌙 章节通关——全队休养,生命回满。','#7fe0a0'); // 章末回满阀门
      run.chapter++;buildMap();log(`—— 进入 <b>${CH_NAME[run.chapter]}</b>，敌人更强了 ——`,'#ffcf5a');const _cs=(typeof showCutscene==='function'&&typeof STORY!=='undefined')?()=>showCutscene([{name:'',pid:null,text:STORY.chapter[run.chapter]}],enterChapterMap):enterChapterMap;setTimeout(_cs,650);return;}
    setTimeout(showMap,500);};
  battleDraft(n,cont);}
function runOver(win){if(run._over)return;run._over=true;window.inBossBattle=false;stage='over';if(typeof sfxWin==='function'){win?sfxWin():sfxLose();}show('endScreen',true);const earned=awardShards(win);try{localStorage.removeItem(SAVE_KEY);}catch(e){}
