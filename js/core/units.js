...
function mkEnemy(t,x,y,boss){const chS=CH_SCALE[run.chapter];const at=(run&&run.ascSel)||0;const aE=ascEnemyMul(at),aB=ascBossMul(at);
  const hp=boss?Math.round(t.hp*BOSS_HP*aB):Math.round(t.hp*chS*ENEMY_POWER*aE);
  const atk=boss?Math.round(t.atk*ENEMY_POWER*aE):Math.round(t.atk*chS*ENEMY_POWER*aE);
  const def=boss?t.def:Math.round(t.def*chS);
  return{id:uid++,name:t.name,pid:t.pid,type:t.type,em:t.em,side:'enemy',x,y,maxhp:hp,hp,atk,def,spd:t.spd,skl:t.skl,lck:t.lck,mov:t.mov,rng:t.rng,elite:!!t.elite,mech:t.mech,dmgCap:t.dmgCap,bossShield:t.bossShield,acted:false,moved:false,transformed:false,shield:0,lv:1,exp:0,skills:t.skills.slice()};}
function mkBattleUnit(src,x,y){const ch=Math.max(1,Math.min(src.maxhp,src.curHp!=null?src.curHp:src.maxhp));return{id:uid++,src,name:src.name,pid:src.pid,type:src.type,em:src.em,side:'player',x,y,maxhp:src.maxhp,hp:ch,atk:src.atk,def:src.def,spd:src.spd,skl:src.skl,lck:src.lck,mov:src.mov,rng:src.rng,elite:false,hero:src.hero,key:src.key,stage:src.stage||0,acted:false,moved:false,transformed:false,shield:0,lv:src.lv,exp:src.exp,skills:src.skills.slice()};}
