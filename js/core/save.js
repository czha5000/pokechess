...
function serializeRun(){return JSON.stringify({pool:run.pool,map:run.map,cur:run.cur,depth:run.depth,chapter:run.chapter,reviveUsed:!!run.reviveUsed,relicIds:run.relics.map(r=>r.id),uid});}
function applyRunData(o){run.pool=o.pool;run.map=o.map;run.cur=o.cur;run.depth=o.depth||6;run.chapter=o.chapter;run.reviveUsed=!!o.reviveUsed;run.relics=(o.relicIds||[]).map(id=>RELICS.find(r=>r.id===id)).filter(Boolean);if(o.uid)uid=Math.max(uid,o.uid);}
...
let retrySnap=null;
function snapshotForRetry(){retrySnap={pool:JSON.parse(JSON.stringify(run.pool)),map:JSON.parse(JSON.stringify(run.map)),cur:run.cur,chapter:run.chapter,reviveUsed:run.reviveUsed,relicIds:run.relics.map(r=>r.id)};}
function retryBattle(){if(!retrySnap){return;}
  ...
  run.cur=retrySnap.cur;run.chapter=retrySnap.chapter;run.reviveUsed=retrySnap.reviveUsed;
