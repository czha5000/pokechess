...
async function aiAct(e){if(e.mech==='enrage'&&!e._enraged&&e.hp/e.maxhp<0.5){e.atk=Math.round(e.atk*1.5);e._enraged=true;log(`⚠ ${e.name} 暴怒！攻击大涨`,'#ff8a8a');floatText(e.x,e.y,'暴怒!','#ff5252');}if(e.bossShield){e.shield=(e.shield||0)+e.bossShield;floatText(e.x,e.y,'+'+e.bossShield+'🛡','#9fd3ff');}let ts=units.filter(u=>u.side==='player'&&u.hp>0);if(!ts.length)return;const hit=ts.filter(t=>aiScore(e,t)>0);if(hit.length)ts=hit;
  ...
  if(Math.abs(e.x-tgt.x)+Math.abs(e.y-tgt.y)<=reach){await strike(e,sk,tgt);if(tgt.hp<=0)killUnit(tgt);
    else{const dd=Math.abs(e.x-tgt.x)+Math.abs(e.y-tgt.y);if(dd<=tgt.rng&&e.hp>0&&!(tgt.eff&&tgt.eff.para)){const back=await strike(tgt,'basic',e);if(e.hp<=0)killUnit(e);else gainExp(tgt,back);}}}}
