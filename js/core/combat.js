...
function pushTarget(att,def,n){let dx=Math.sign(def.x-att.x),dy=Math.sign(def.y-att.y);if(dx&&dy){if(Math.abs(def.x-att.x)>=Math.abs(def.y-att.y))dy=0;else dx=0;}if(!dx&&!dy)return;for(let i=0;i<n;i++){const nx=def.x+dx,ny=def.y+dy;if(nx<0||ny<0||nx>=COLS||ny>=ROWS||TERRAIN[ny][nx]===1){floatText(def.x,def.y,'坠落!','#43c6ff');burst(def.x,def.y,'#43c6ff');if(typeof sfxDrown==='function')sfxDrown();if(typeof vfxHit==='function')vfxHit(def.x,def.y,'water',false,true);def.hp=0;killUnit(def);return;}const occ=unitAt(nx,ny);if(occ){def.hp-=4;occ.hp-=4;floatText(def.x,def.y,'撞击-4','#ffd95a');if(occ.hp<=0)killUnit(occ);if(def.hp<=0)killUnit(def);return;}def.x=nx;def.y=ny;}}
function baseDmg(att,def,sk){const m=typeMult(sk.type,def.type);if(m===0)return{d:0,m:0};let d=Math.max(1,Math.round(att.atk*sk.mult*m)-def.def-coverOf(def));d=Math.round(d*relicDmgMult(att,sk,def)*relicDmgTaken(def));if(sk.useShield)d+=(att.shield||0);if(TERRAIN[att.y]&&TERRAIN[att.y][att.x]===4)d=Math.round(d*1.25);if(isFlankedSide(att,def))d=Math.round(d*((att.side==='player'&&hasRelic('flank'))?1.5:1.15));if(att.side==='player'){if(hasRelic('formation'))d=Math.round(d*(1+0.12*adjAllies(att)));if(hasRelic('alpha')&&!def.acted)d=Math.round(d*1.4);}return{d:Math.max(1,d),m};}
function hitRate(att,def,sk){return Math.max(0,Math.min(100,Math.round(sk.hit+att.skl*2-(def.spd*2+def.lck)-avoidTerrain(def)+relicHitAdd(att,sk)+(itemOf(att)?(itemOf(att).hit||0):0))));}
...
async function execAttack(att,sk,tgt){if(busy)return;busy=true;const s=SKILLS[sk];clearSel();
  if(s.kind==='aoe'){const list=[tgt,...units.filter(e=>e.side==='enemy'&&e.hp>0&&e!==tgt&&Math.abs(e.x-tgt.x)+Math.abs(e.y-tgt.y)===1)];let dealt=0;await lunge(att,tgt.x,tgt.y);bw.classList.add('shake');setTimeout(()=>bw.classList.remove('shake'),320);
    for(const d of list){const b=baseDmg(att,d,s);if(b.m===0){floatText(d.x,d.y,'无效','#cfcfcf');continue;}if(rand100()>=hitRate(att,d,s)){floatText(d.x,d.y,'Miss','#cfcfcf');continue;}const ab=Math.min(d.shield||0,b.d);if(ab>0)d.shield-=ab;d.hp-=(b.d-ab);dealt+=b.d;flashCell(d.x,d.y);burst(d.x,d.y,b.m>1?'#7fe0a0':'#fff');floatText(d.x,d.y,'-'+b.d+(b.m>1?'!':''),b.m>1?'#7fe0a0':'#fff');setHp(d);if(typeof vfxHit==='function')vfxHit(d.x,d.y,s.type,false,b.m>1);}
