...
function pushTarget(att,def,n){let dx=Math.sign(def.x-att.x),dy=Math.sign(def.y-att.y);if(dx&&dy){if(Math.abs(def.x-att.x)>=Math.abs(def.y-att.y))dy=0;else dx=0;}if(!dx&&!dy)return;for(let i=0;i<n;i++){const nx=def.x+dx,ny=def.y+dy;if(nx<0||ny<0||nx>=COLS||ny>=ROWS||TERRAIN[ny][nx]===1){floatText(def.x,def.y,'坠落!','#43c6ff');burst(def.x,def.y,'#43c6ff');if(typeof sfxDrown==='function')sfxDrown();def.hp=0;killUnit(def);return;}const occ=unitAt(nx,ny);if(occ){def.hp-=4;occ.hp-=4;floatText(def.x,def.y,'撞击-4','#ffd95a');if(occ.hp<=0)killUnit(occ);if(def.hp<=0)killUnit(def);return;}def.x=nx;def.y=ny;}}
function baseDmg(att,def,sk){const m=typeMult(sk.type,def.type);if(m===0)return{d:0,m:0};let d=Math.max(1,Math.round(att.atk*sk.mult*m)-def.def-coverOf(def));d=Math.round(d*relicDmgMult(att,sk,def)*relicDmgTaken(def));if(sk.useShield)d+=(att.shield||0);if(att.side==='player'){if(hasRelic('formation'))d=Math.round(d*(1+0.12*adjAllies(att)));if(hasRelic('flank')&&isFlanked(att,def))d=Math.round(d*1.5);if(hasRelic('alpha')&&!def.acted)d=Math.round(d*1.4);}return{d:Math.max(1,d),m};}
...
async function strike(att,sk,def){const s=SKILLS[sk]||sk;const times=(doubles(att,def)&&s.kind!=='aoe')?2:1;let dealt=0;
  for(let i=0;i<times;i++){if(def.hp<=0)break;await lunge(att,def.x,def.y);const b=baseDmg(att,def,s);
    ...
    const isCrit=rand100()<critRate(att,def,s);let d=isCrit?b.d*CRITX:b.d;if(def.dmgCap&&d>def.dmgCap){d=def.dmgCap;floatText(def.x,def.y,'硬鳞!','#9fd3ff');}const ab=Math.min(def.shield||0,d);if(ab>0){def.shield-=ab;floatText(def.x,def.y,'-'+ab+'🛡','#9fd3ff');}def.hp-=(d-ab);dealt+=d;if(typeof sfxHit==='function')sfxHit(isCrit,b.m>1);
...
async function execAttack(att,sk,tgt){if(busy)return;busy=true;const s=SKILLS[sk];clearSel();
  if(s.kind==='aoe'){const list=[tgt,...units.filter(e=>e.side==='enemy'&&e.hp>0&&e!==tgt&&Math.abs(e.x-tgt.x)+Math.abs(e.y-tgt.y)===1)];let dealt=0;await lunge(att,tgt.x,tgt.y);bw.classList.add('shake');setTimeout(()=>bw.classList.remove('shake'),320);
    for(const d of list){const b=baseDmg(att,d,s);if(b.m===0){floatText(d.x,d.y,'无效','#cfcfcf');continue;}if(rand100()>=hitRate(att,d,s)){floatText(d.x,d.y,'Miss','#cfcfcf');continue;}const ab=Math.min(d.shield||0,b.d);if(ab>0)d.shield-=ab;d.hp-=(b.d-ab);dealt+=b.d;flashCell(d.x,d.y);burst(d.x,d.y,b.m>1?'#7fe0a0':'#fff');floatText(d.x,d.y,'-'+b.d+(b.m>1?'!':''),b.m>1?'#7fe0a0':'#fff');setHp(d);}
  ...
  else{const dist=Math.abs(att.x-tgt.x)+Math.abs(att.y-tgt.y);if(dist<=tgt.rng&&att.hp>0&&!(tgt.eff&&tgt.eff.para)){await delay(110);await strike(tgt,'basic',att);if(att.hp<=0)killUnit(att);}gainExp(att,Math.round(dealt*1.4));}
  busy=false;finishAct(att);}
function execHeal(att,sk,tgt){if(busy)return;const s=SKILLS[sk];clearSel();floatText(tgt.x,tgt.y,'+'+s.amount,'#6affa0');burst(tgt.x,tgt.y,'#6affa0');tgt.hp=Math.min(tgt.maxhp,tgt.hp+s.amount);setHp(tgt);log(`${att.name}【治疗】恢复 ${tgt.name} ${s.amount}`,'#6affa0');if(typeof sfxHeal==='function')sfxHeal();att.acted=true;render();advanceInit();}
