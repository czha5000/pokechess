function terrainTag(x,y){const t=TERRAIN[y][x];return t===4?'<span style="color:#caa765">⛰高地(造成伤害+25%)</span>':t===2?'<span style="color:#7fe0a0">🌲森林(受伤−2·闪避+15%)</span>':t===3?'<span style="color:#ff8a5a">🔥岩浆(每回合−4)</span>':'平地(无效果)';}
// 侧栏:单位信息、技能栏、行动按钮、伊布进化、战斗预测面板
function renderSkills(u){const row=document.getElementById('skillRow');row.innerHTML='';
  if(u._charged){const c=document.createElement('div');c.className='small';c.style.cssText='color:#ffd95a;font-weight:700';c.textContent='⚡ 蓄力中:下次攻击 ×'+(typeof CHARGE_MULT!=='undefined'?CHARGE_MULT:1.8);row.appendChild(c);}
  u.skills.forEach(sk=>{const s=SKILLS[sk];const b=document.createElement('button');b.className='btn skbtn'+(pendingSkill===sk?' on':'');
    b.innerHTML=`<span><span class="tdot" style="background:${s.type?TCOLOR[s.type]:'#bbb'}"></span>${s.name}${(u.skillUp&&u.skillUp[sk])?' <b style="color:#ffd95a">+'+u.skillUp[sk]+'</b>':''}</span><small>${s.desc}</small>`;
    b.onclick=()=>{if(pendingSkill===sk){pendingSkill=null;pendingTgt=null;fcEl.innerHTML='';showOwnRange(u);renderSkills(u);renderActs(u);render();}else{pendingSkill=sk;pendingTgt=null;fcEl.innerHTML='';highlightTargets(u,sk);renderSkills(u);render();}};row.appendChild(b);});}
function highlightTargets(u,sk){const s=SKILLS[sk];highlights=[];
  if(s.kind==='charge'){highlights.push({x:u.x,y:u.y,kind:'heal'});return;} // 自身:点自己确认
  if(s.kind==='swap'){units.filter(a=>a.side===u.side&&a.hp>0&&a!==u&&Math.abs(a.x-u.x)+Math.abs(a.y-u.y)<=(u.rng+(s.rb||0))).forEach(a=>highlights.push({x:a.x,y:a.y,kind:'heal'}));return;}
  ...
  else{const reach=u.rng+(s.rb||0);units.filter(e=>e.side==='enemy'&&e.hp>0).forEach(e=>{const inR=Math.abs(e.x-u.x)+Math.abs(e.y-u.y)<=reach;highlights.push({x:e.x,y:e.y,kind:inR?'atk':'oor'});});}}
function renderActs(u){const row=document.getElementById('actRow');row.innerHTML='';
  if(u.moved&&!u.acted&&!u._noUndo){const ub=document.createElement('button');ub.className='btn ghost';ub.textContent='↩ 撤销移动(或右键)';ub.onclick=()=>undoMove(u);row.appendChild(ub);}
  ...
  const _canRest=(typeof REST_HEAL!=='undefined'&&REST_HEAL>0&&u.hp<u.maxhp&&!units.some(e=>e.side!==u.side&&e.hp>0&&Math.abs(e.x-u.x)+Math.abs(e.y-u.y)<=1));
  const w=document.createElement('button');w.className='btn ghost';
  w.textContent=_canRest?('待机并喘息 +'+Math.max(1,Math.round(u.maxhp*REST_HEAL))):'待机(结束该单位)';
  w.onclick=()=>{if(!w.dataset.armed){w.dataset.armed='1';w.textContent='再点一次=确认';w.style.background='var(--accent)';return;}
    if(typeof tryRestHeal==='function')tryRestHeal(u);u.acted=true;clearSel();render();advanceInit();};row.appendChild(w);}
...
function showInfo(u){const atMax=u.lv>=MAXLV,need=THRESH[u.lv]||0,rem=atMax?0:Math.max(0,need-u.exp);
  ...
  document.getElementById('info').innerHTML=`<b>${u.name}</b> <span class="pill" style="background:${TCOLOR[u.type]}">${TYPE_CN[u.type]}</span> Lv.${u.lv}${u.hero?' ·主角':''}<br>HP ${Math.max(0,u.hp)}/${u.maxhp}　攻 ${u.atk}　防 ${u.def}<br>速 ${u.spd}　技 ${u.skl}　幸 ${u.lck}　移 ${u.mov}　射程 ${u.rng}
    ...
    <div class="small">脚下地形：${terrainTag(u.x,u.y)}</div>
...
function showForecast(att,sk,tgt){pendingTgt=tgt;let s=SKILLS[sk];{const _up=(att.skillUp&&att.skillUp[sk])||0;if(_up)s=Object.assign({},s,{mult:s.mult*(1+0.2*_up)});}const a=baseDmg(att,tgt,s),aHit=hitRate(att,tgt,s),aCrit=critRate(att,tgt,s),aDbl=doubles(att,tgt)&&s.kind!=='aoe';
  ...
  const cancel=document.createElement('button');cancel.className='btn ghost';cancel.textContent=att.moved?'取消':'取消(退回可移动)';cancel.onclick=()=>{pendingSkill=null;pendingTgt=null;fcEl.innerHTML='';showOwnRange(att);renderSkills(att);renderActs(att);render();};row.appendChild(cancel);render();}
