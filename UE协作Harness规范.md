...
4. **生成粘贴块前查备忘录**,别对函数名/pin 类型现猜——猜错的代价是节点被静默丢弃或编译报错,通常要再来一整轮才能发现。
   - **生成粘贴块一律走 pin 注册表生成器,禁止手写大段 K2Node 文本**:优先用本机 `ue-blueprint-paste-gen` skill;若 Cloud 环境没有该 skill,用仓库内 `ue/tools/paste_gen.py`(同样先登记 pin、再连线、校验 LinkedTo 全部可解析后才输出)。这条规则是从多轮"连线看着对、实际类型没解析上"的返工里总结出来的,是硬性要求,不是建议。
