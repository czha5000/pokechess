...
**2026-09-01 起区分「必读」与「按需查」**——此前把 542KB 全列为必读,实际必然退化成"只读最近几节 + grep",反而是 harness 想防止的失败模式(详见 `工程评估报告.md` 第五节)。现在必读量约 130KB(原 542KB 的 24%):

| 文件 | 作用 | 必读? | 更新频率 |
|---|---|---|---|
| `UE硬规则.md` | 86 条踩坑的**纯结论提炼**(原备忘录的"硬规则速查表"抽出来的,31KB) | ✅ **必读** | 提炼出新的通用规则时 |
| `UE蓝图状态.md` 的 `## BP_*`/`## WBP_*`/`## 系统` 章节 | 每个蓝图的变量/函数/GUID/EventGraph 结构快照(97KB) | ✅ **必读** | **每次改动蓝图后** |
| `UE规则对齐表.md` | UE 与 web 的规则逐条对照,每条标 已对齐/故意简化/待办/不一致 | ✅ **改战斗规则前必读** | 每次新增或改动战斗规则 |
| `UE测试用例.md` | 功能验收清单(PASS/FAIL) | ✅ **验收前必读相关项** | 每次新增/验证功能 |
| `UE节点备忘录.md` | 案例档案:每条坑的完整现象/根因/修法(261KB) | 🔍 按"坑XX"编号 grep | 每次踩到新坑 |
| `UE蓝图状态.md` 的 `### 2026-xx-xx` 日期章节 | 改动史/排查过程存档(156KB) | 🔍 排查时 grep | 每次改动后追加 |
| `UE实操教程.md` | 总体教程+进度追踪,顶部"续接区"是新会话的入口 | 🔍 按需 | 每完成一个切片步骤 |
| `UE协作Harness规范.md` | 本文件,协议本身 | 🔍 按需 | 协议变化时 |
| `UE美术管线.md` / `UE过场动画管线.md` | 美术/动画相关的独立手册,和蓝图协作无关 | 🔍 按需 | 按需 |
| `ue-add-animation/SKILL.md` | 给角色新增/替换动画的操作 SOP | 🔍 加动画时必读 | 流程本身变化时 |
| `ue/tools/paste_gen.py` + `ue/README.md` | 粘贴块生成器(pin 注册表 + 连线完整性校验) | 🔍 大范围图重构时 | 工具变化时 |
...
1. 读 **`UE硬规则.md`**(31KB)—— 避免重犯已知错误(函数名、pin 写法、DSL 陷阱等)。**不要通读 `UE节点备忘录.md`**,那是 261KB 的案例档案,需要某条细节时拿"坑XX"编号去 grep。
2. 读 **`UE蓝图状态.md` 的 `## BP_*`/`## WBP_*`/`## 系统` 快照章节** —— 知道现有蓝图的变量/函数/GUID,**不要求用户重新粘贴现状**。同一份文件里 `### 2026-xx-xx` 的日期章节是改动史,不必通读,排查时再 grep。
3. 读 `UE实操教程.md` 的续接区 —— 知道整体做到第几步。
4. 要动战斗规则(伤害/命中/反击/技能效果/状态)的话,查 **`UE规则对齐表.md`** 对应行 —— 确认这次改动会不会和 web 版产生不一致。
5. 看 `UE测试用例.md` 的相关项 —— 知道哪些功能已验证、哪些还没测。
6. 只有当文档信息不足以判断问题时,才要求用户提供当前蓝图的实际粘贴文本(且只要相关节点,不要整图,见第4条原则)。
