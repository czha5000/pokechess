...
- [ ] **遗物生效**:战斗一开始(`BeginPlay`)我方单位 `Atk`/`Def` 应该比 Class Default(10/5)高(切片默认 5 件:`power_band` Atk+2 等)。顶栏应显示遗物名,不再只能靠手算伤害间接确认
- [ ] 敌方 AI 攻击仍然只用普通攻击效果(没有元素技能/属性克制加成),这是已知简化不是遗漏

## 遗物顶栏 + Debug 控制台(2026-08-16)
- [ ] Play 后顶栏应显示中文全名（力量头带 | 钢之意志 | …），不是 `power_band`
- [ ] Debug 面板技能/遗物应是下拉框（全中文名；诅咒带「（诅咒）」；遗物第一项「（空）」）
- [ ] 点 DBG → 屏幕正中「选择配装」,能看到输入框 + **1 切片 / 2 重击流 / 3 元素流 / 应用自定义配装 / 关闭**
- [ ] 点 **2 重击流** → 顶栏出现玻璃大炮/力量头带等,Output Log 有 `Loadout HEAVY skills=`
- [ ] 移动我方单位,行动菜单五个技能按钮应显示 **重击 / 强袭 / 近身战 / 铁尾 / 咬碎击**,不是英文 id
- [ ] 重击流的强袭/近身战打满血敌人,预计伤害应明显高于切片的普通攻击(约 0.9 倍率 vs 1.7)
- [ ] 再点 **1 切片**,菜单应变回 **普通攻击/重击/火花/水枪/藤鞭**,顶栏回到切片 5 遗物
...
- [x] 手填 CSV 只靠 Details 默认 Text:**已废弃**(GetText 经常空)。现改为 EventConstruct 运行时填下拉 + `SetText` 隐藏框,再 APPLY。下拉空 + `RelicCsvToIdsB` prune 会把遗物写成 0 件,看起来像没 APPLY。
