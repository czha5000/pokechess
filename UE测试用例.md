...
## 行动菜单(2026-08-16 新增,移动后弹"攻击/待命/撤销")
- [ ] **移动后弹出菜单**:点自己单位→移动到高亮格→高亮清除,屏幕上 `(500,400)` 位置附近应该出现三个按钮"攻击"/"待命"/"撤销"
- [ ] **菜单按钮真的能被点击**(本条是最大的不确定项,MCP 没有能力验证——`ShowActionMenu` 里切到了 `SetInputModeGameAndUI`,但没做过真人 Play 验证输入路由是否真的生效,可能出现"菜单显示了但点哪个按钮都没反应"这种情况,如果遇到大概率是输入模式切换没生效或者顺序问题,见 `UE节点备忘录.md` 坑19)
- [ ] 点"攻击" → 菜单消失,接着点场上敌方单位应该和之前一样正常触发 `TryAttack`(命中自动结束回合,未命中允许再点近一点的敌人)
- [ ] 点"待命" → 菜单消失,直接结束回合(不用攻击),`CurrentIndex` 推进到下一个单位,行动顺序条同步刷新
- [ ] 点"撤销" → 菜单消失,单位视觉位置和逻辑 Col/Row 都回到本回合开始时的位置,并且可以**重新点自己再移动一次**(`bHasMoved` 也被撤销重置为 false,不是"这个回合彻底废了")
